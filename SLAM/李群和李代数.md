...
\end{align}
$$
这里$\cal{J}$ 是一个6*6 的矩阵，形式较为复杂且在实际计算中并不用到改矩阵，所以略去其具体形式。
...
另一种方法就是先对旋转矩阵$\mathbf{R}$ 进行一次扰动$\Delta\mathbf{R}$，然后直接对这个扰动进行求导。这里以左乘扰动模型为例。设左扰动的李代数为$\varphi$，直接对$\varphi$（而不是$\mathbf{R}$）进行求导：
$$
...
\begin{bmatrix}
\mathbf{I}_{3\times3} & -(\mathbf{R}\mathbf{p}+\mathbf{t})^\wedge_{3\times3} \\
\mathbf{0}^T_{1\times3} & \mathbf{0}^T_{1\times3}\\
\end{bmatrix}=(\mathbf{T}\mathbf{p})^\bigodot
$$
